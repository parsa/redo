...
# Bulk deps input test (NUL-separated).
# Same intent as test_ifchange_fromfile.sh but uses --from-file0.
...
cat >all.do <<'EOF'
# Build deps from a NUL-separated list.
printf 'a\0b\0inc dir/some header.h\0' >deps0

# One bulk call; must preserve spaces in the filename.
redo-ifchange --from-file0 deps0
...
run_redo || {
  echo "FAIL: expected redo -j2 all (with redo-ifchange --from-file0) to succeed" >&2
  exit 10
}
